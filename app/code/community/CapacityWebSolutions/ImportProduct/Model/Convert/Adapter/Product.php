...
class CapacityWebSolutions_ImportProduct_Model_Convert_Adapter_Product extends Mage_Catalog_Model_Convert_Adapter_Product
{
     ...
     */
      ...
      public function getImageFromURL($f_image){
          
          
          
                      
          
          if (false===strpos($f_image,"http"))
          {
             return $f_image; 
          }
          ...
          $imageURL = $f_image;
        $params = explode("/",$imageURL);
        $filename  = array_pop($params);
        
        $newFileName =   $filename;
        
        $fullPath =   Mage::getBaseDir()."/media/import/".$newFileName;
    ...
    public function saveRow(array $importData)
    {
        $product = $this->getProductModel()
         ...
         $newFileName  = $this->getImageFromURL($importData['image']);  
         $product->addImageToMediaGallery(Mage::getBaseDir()."/media/import/".$newFileName,"image");   
        ...
        $fromDate = Mage::app()->getLocale()->date()->toString(Varien_Date::DATETIME_INTERNAL_FORMAT);
        $toDate = date("Y-m-d G:i:s", strtotime($fromDate) + 60 * 60 * 24 * 30 * 3);
        $product->setStoreId($store->getId());
        $product->setNewsFromDate($fromDate);
        $product->setNewsToDate($toDate);
