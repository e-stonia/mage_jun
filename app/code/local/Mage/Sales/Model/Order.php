...
class Mage_Sales_Model_Order extends Mage_Sales_Model_Abstract
{
    ...
    private $_totalTax = 0.00;
    private $_totalPantCost = 0.00;
...
/*********************** STATUSES ***************************/
    ...
    /**
    * Calculate Totals with prices of botlles and TAX 
    * 
    */
    
    function getTotalPantCost(){
        return $this->_totalPantCost;
    }
    ...
    function getTotalWithTaxAndPant(){
         return $this->getGrandTotal()+$this->_totalPantCost+$this->_totalTax;
    }
    function calculateTotals(){
        
        $data = array();      
              foreach($this->getItemsCollection() as $item)
              {    //d($item->getQtyOrdered());
                  //d($item->toArray());
                  $product = Mage::getModel('catalog/product')->load($item->getProductId());
                  $data[$item->getProductId] = array(
                    'tax_value' => $product->getTaxValue(),
                    'pant' => $product->getPant()
                  );
                  $tax_value = 0.00;
                  $pant = 0.00;
                  $tax_value = $product->getTaxValue();
                 ...
                 $pant = $product->getPant();
                  if ($pant>0.00)
                      $this->_totalPantCost +=floatval($item->getQtyOrdered())*floatval($pant);
                  if ($tax_value)
                  {
                      $this->_totalTax += $item->getQtyOrdered()*(($product->getFinalPrice()/100)*$tax_value);
                  }

              }
             // dd(array(
//                '1'=>$this->getGrandTotal(),
//                '2'=>$this->_totalPantCost,
//                '3'=>$this->_totalTax,
//                "4"=>
//              ));
//die("DONE");
