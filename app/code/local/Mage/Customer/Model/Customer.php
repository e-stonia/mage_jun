...
//        }
    ...
    public function getData($param = null){
    	if ($param=='name') return $this->getFirstname()." ".$this->getLastname();
    	if ($param)
...
//            }
    ...
    public function save(){
        ...
        if(!isset($_POST['usercomment'])){
            $this->addEmailToSendsmaily();
        }
        return ret;
    //	
    }
    
   public function addEmailToSendsmaily(){
   	
       //d($_POST);
       
  
   
    	$ch = curl_init("https://horeca.sendsmaily.net/api/opt-in/");
    	$vars = array(
    		'autoresponder'=>16,
    		"key"=>"ItwIP6NFRRqxYLOlQmi-U8u6FeZ9H0ftfz7kBjGYypY,",
    		"eesnimi"=>$this->getFirstname(),
    		"perenimi" =>$this->getLastname(),
    		"firma"=>$_POST['company'],
    		"email"=>$this->getEmail()
    ...
    public function getManager(){
    	$collection = Mage::getModel("manager/manager")->getCollection();
	  	$collection->getSelect()->where("code = ?",$this->getData("middlename"));
	  	//d($collection->count());
    	//d(get_class_methods(get_class($collection)));
    	try {
