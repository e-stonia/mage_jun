...
class Mage_Adminhtml_Catalog_ProductController extends Mage_Adminhtml_Controller_Action
{
     ...
     * @var array
     */
    protected $_publicActions = array('edit','getimage');
    ...
    public function getimageAction()
    {
        ...
        $imageURL = $_GET['external-url'];
        $params = explode("/",$imageURL);
        $filename  = array_pop($params);
        
        $fullPath =   Mage::getBaseDir()."/var/tmp/".time()."_".$filename;
        ...
        $product = $this->_initProduct();
        $product->addImageToMediaGallery($fullPath);
        $product->save();
