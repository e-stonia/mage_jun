...
class <Namespace>_<Module>_Adminhtml_<Module>Controller extends Mage_Adminhtml_Controller_action
{
	...
	public function editAction() {
		...
		if ($model->getId() || $id == 0) {
			...
			Mage::register('<module>_data', $model);
    ...
    public function massDeleteAction() {
        $<module>Ids = $this->getRequest()->getParam('<module>');
        if(!is_array($<module>Ids)) {
			Mage::getSingleton('adminhtml/session')->addError(Mage::helper('adminhtml')->__('Please select item(s)'));
        } else {
            try {
                foreach ($<module>Ids as $<module>Id) {
                    $<module> = Mage::getModel('<module>/<module>')->load($<module>Id);
                    $<module>->delete();
                }
                Mage::getSingleton('adminhtml/session')->addSuccess(
                    Mage::helper('adminhtml')->__(
                        'Total of %d record(s) were successfully deleted', count($<module>Ids)
                    )
                );
            } catch (Exception $e) {
                Mage::getSingleton('adminhtml/session')->addError($e->getMessage());
    ...
    public function massStatusAction()
    {
        ...
        if(!is_array($<module>Ids)) {
            ...
            try {
                foreach ($<module>Ids as $<module>Id) {
                    $<module> = Mage::getSingleton('<module>/<module>')
                        ->load($<module>Id)
                ...
                $this->_getSession()->addSuccess(
                    $this->__('Total of %d record(s) were successfully updated', count($<module>Ids))
                );
            } catch (Exception $e) {
    ...
    {
        $fileName   = '<module>.csv';
        $content    = $this->getLayout()->createBlock('<module>/adminhtml_<module>_grid')
            ->getCsv();
